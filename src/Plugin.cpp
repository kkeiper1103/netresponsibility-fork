...
/**
 * // I *think* this was a non-functional feature that hadn't been implemented when the project was abandoned
#include "Plugin.h"

// this is a source file, so I guess it's not as bad??
using namespace std;
...
void Report::makeWarningsSection(string &suspicious) {
	...
	if (warnings.size() > 0) {
				...
				k != tree.end(); k++)
		{
					...
					id != k->second.end(); id++)
			{
				...
				attUrlsContent <<"['" <<warnings.getAbbrUrl(*id)
						<<"', ['http://" <<warnings.getUrl(*id) <<"'],, '"
						<<warnings.getDateTime(_options->getTxt("dateTimeFormat"),*id)
		...
		keywordsContent = attKeywordsContent
				= _options->getTxt("reportNoWarnings");
	...
	if (_options->isAttachedReportPart("warnings"))
		_attached <<makeJavascriptBranch(
				_options->getTxt("reportWarningsTitle"), attKeywordsContent);
}
...
void Report::makeWhitelistSection() {
	...
	if (whitelist.size() > 0) {
		...
		while (whitelist.hasMore()) {
			...
			attUrlsContent += "['" + whitelist.getAbbrUrl() + "', ['http://"
					+ whitelist.getUrl() + "'],, '"
					+ whitelist.getDateTime(_options->getTxt("dateTimeFormat"))
					+ "']";
			whitelist.next();
		...
		if (_options->isAttachedReportPart("whitelist"))
			_attached <<makeJavascriptBranch(
					_options->getTxt("reportWhitelistTitle"), attUrlsContent);
	}
}
					...
					it2 != hostnames.end(); it2++)
			{
				if (doIncludePaths) {
					...
					while (history.hasMore()) {
						pathsContent += "['" + history.getUrl() + "', ['http'],, '"
								+history.getDateTime(_options->getTxt("dateTimeFormat"))
								+ "'],\n";
						history.next();
					}
					...
					domainsContent += "['" + *it2 + "', ['http',,'folder'],, ''],\n";
			}
			historyContent += makeJavascriptBranch(*it, domainsContent);
		}
	}
	else
		historyContent = _options->getTxt("reportNoHistory");
	if (_options->doSaveHistory())
		_attached <<makeJavascriptBranch(_options
